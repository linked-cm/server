import { relativeFileSystemUploadPath, getUploadTarget, } from '@_linked/server-utils/utils/Upload';
import { Shape } from '@_linked/core/shapes/Shape';
import * as fsSync from 'node:fs';
import fs from 'node:fs/promises';
import path from 'node:path';
export class LocalFileStore extends Shape {
    constructor(n, basePath = relativeFileSystemUploadPath) {
        if (typeof n === 'string') {
    ...
    async listFiles(prefix) {
        ...
        const allFiles = await Promise.all(files.map((file) => {
            ...
            return fs
                ...
                .then((stat) => {
                if (stat.isDirectory()) {
                    return this.listFiles(prefix).then((files) => files.map((file) => path.join(filePath, file)));
                }
                else {
                    return [filePath];
                }
     ...
     */
    async saveFile(filePath, fileContent, mimeType) {
        const { publicURL, targetFilePath } = getUploadTarget(filePath, mimeType, null, '', this.accessURL);
