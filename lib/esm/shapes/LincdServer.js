...
import { getWebpackAppConfig } from 'lincd-cli/config-webpack-app';
import { getLincdPackages } from 'lincd-cli/cli-methods';
import { getPackageJSON } from 'lincd-cli/utils';
...
let LincdServer = class LincdServer extends Shape {
    ...
    async start() {
        ...
        const staticAccessURL = (process.env.STATIC_ACCESS_URL ||
     ...
     */
    async initStores() {
        return Promise.all(LinkedStorage.getStores().map((store) => {
